...
def predict():
    ...
    global lr
    if request.method == 'POST':
        out = lr.predict([[float(day), float(visibilityHigh), float(visibilityAvg), float(month), float(tempHigh),
                           float(tempAvg), float(visibilityLow), float(tempLow), float(windAvg), float(DPLow),
                           float(DPHigh),
                           float(DPAvg), float(humidityHigh),
                           float(SLPHigh), float(SLPLow), float(SLPAvg), float(humidityAvg), float(humidityLow)]])
        #out(0, abs(out[0]))
        flash(f'The Rainfall  is{out}')
        #return out
        #output.delete(0, END)
